    ...
    def step(self, action):
        ...
        for step in range(self.n_action_steps):
            ...
            if len(self.done) > 0 and self.done[-1]:
                # Episode already ended inside this chunk.
                break
            ...
            self.done.append(episode_done)
            self._add_info(info)

        observation = self._get_obs(self.video_delta_indices, self.state_delta_indices)
        reward = aggregate(self.reward, self.reward_agg_method)
        terminated = aggregate(terminations, "max") if terminations else False
        truncated = aggregate(truncations, "max") if truncations else False
        ...
        info["actions"] = action
        info["terminateds"] = terminations
        info["truncateds"] = truncations
        info["dones"] = episode_dones
        return observation, reward, terminated, truncated, info

    def clear_rollout_tracking(self):
        """Reset reward, done, and info caches while keeping the latest observation history."""
        self.reward = list()
        self.done = list()
        self.info = defaultdict(lambda: deque(maxlen=self.max_steps_needed + 1))
